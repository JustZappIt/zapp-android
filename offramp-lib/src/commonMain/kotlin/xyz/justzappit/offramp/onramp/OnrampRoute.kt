...
data class OnrampRouteLimits(
    ...
    /**
     * What the amount screen shows and the gate opens on. Ignores the daily count on purpose: a
     * selfie wallet that has used today's orders must be told it reached today's limit, not that
     * buying is unavailable.
     */
    ...
    fun decide(netUsdc: Usdc6): OnrampRouteDecision =
        when {
            ...
            else -> OnrampRouteDecision.DailyExhausted
        }
}
...
sealed interface OnrampRouteDecision {
    ...
    data object DailyExhausted : OnrampRouteDecision
