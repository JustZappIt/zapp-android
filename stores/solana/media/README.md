...
This directory specifies the media assets for the Solana dApp Store listing.

> **The files are not present.** The previous ones were upstream's artwork and were removed
> rather than republished as this project's own. Supply Zapp artwork matching the spec below,
> then restore the commented-out `media:` block in `../release-notes-template.yaml`. The
> `publish_to_solana` job will fail until both are done.
