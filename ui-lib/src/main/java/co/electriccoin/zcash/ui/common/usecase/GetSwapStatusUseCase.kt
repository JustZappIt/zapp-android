                        ...
                        if (newAssets?.data == null || newAssets.zecAsset == null) {
                            data.update {
                                it.copy(
                                    ...
                                    error =
                                        newAssets?.error
                                            ?: IllegalStateException("No swap assets available")
                                )
                            }
                            return@launch
                        } else {
                ...
                while (true) {
                    try {
                        ...
                        requireMatchingAsset(
                            ...
                            expectedChainTicker = expectedMetadata.origin.chainTicker,
                            actual = result.quote.originAsset
                        )
                        requireMatchingAsset(
                            name = "destination",
                            expectedTokenTicker = expectedMetadata.destination.tokenTicker,
                            expectedChainTicker = expectedMetadata.destination.chainTicker,
