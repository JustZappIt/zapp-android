...
import xyz.justzappit.offramp.p2p.PaymentQrDetector
...
internal class ScanZashiAddressVM(
                ...
                if (!hasBeenScannedSuccessfully) {
                    runCatching {
                        ...
                        when {
                            ...
                            args.flow == ScanFlow.HOMEPAGE -> {
                                onPossiblePaymentQr(result)
                            }
    ...
    // Home scanner only: not a Zcash address / ZIP-321 URI, so try the local payment rails
    // (UPI / PIX / QRIS) and route a hit into the pay-merchant flow carrying the scanned payload.
    private suspend fun onPossiblePaymentQr(result: String) {
        val currency = PaymentQrDetector.detect(result)
        if (currency == null) {
            onInvalidScan()
            return
        }
            ...
            is PaymentQrParseResult.Success -> {
                ...
                navigationRouter.replace(
                    UpiOfframpArgs(
                        ...
                        prescannedPayload = result,
                        prescannedPaymentAddress = parsed.parsed.paymentAddress,
                        prescannedFiatAmount = parsed.parsed.fiatAmount?.toPlainString(),
                    ),
                )
            }

            is PaymentQrParseResult.Failure -> {
                Twig.info { "ScanZashiAddressVM: rejecting ${currency.code} payment QR, error=${parsed.error}" }
                onInvalidScan()
            }
        }
    }
