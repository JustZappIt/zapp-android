...
class BalanceWidgetVM(
    ...
    private fun createState(account: WalletAccount?, exchangeRateUsd: ExchangeRateState) =
        BalanceWidgetState(
            ...
            breakdown = createBreakdown(account),
            onBalanceClick =
                if (args.isBalanceBreakdownEnabled && account != null) ::onBalanceClick else null,
        )

    private fun createBreakdown(account: WalletAccount?): ShieldBreakdownState? =
        when {
            !args.isShieldBreakdownEnabled -> {
                null
            }
            ...
            !account.isShieldingAvailable -> {
                ...
                ShieldBreakdownState(
                    shieldedBalance = account.totalShieldedBalance,
                    transparentBalance = account.totalTransparentBalance,
                    onShieldClick = ::onShieldClick,
                    onBreakdownClick = ::onBalanceButtonClick,
                )
            }
        }

    private fun onAddZecClick() = navigationRouter.forward(TopUpArgs)
...
data class BalanceWidgetArgs(
    ...
    val isShieldBreakdownEnabled: Boolean = false,
