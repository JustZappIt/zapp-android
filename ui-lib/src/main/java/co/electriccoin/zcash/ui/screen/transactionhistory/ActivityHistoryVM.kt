...
class ActivityHistoryVM(
    ...
    val state =
        ...
        ) { (activities, searchHash), filters, exchangeRate, zecUsdPrice ->
            ActivityHistoryInput(activities, filters, searchHash, exchangeRate, zecUsdPrice)
        }.mapLatest { input ->
            val activities = input.activities
            when {
                activities == null -> {
                    createLoadingState(filtersSize = input.filters.size)
                }

                activities.isEmpty() -> {
                    createEmptyState(filtersSize = input.filters.size)
                }
...
private data class ActivityHistoryInput(
    val activities: List<ActivityData>?,
    val filters: List<TransactionFilter>,
    val searchHash: String,
    val exchangeRate: ExchangeRateState,
    val zecUsdPrice: BigDecimal?
)
