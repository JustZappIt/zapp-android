...
class ActivityMapper {
    ...
    /**
     * Shielding consumes transparent funds ([Transaction.amount] == totalSpent) and lands
     * `amount - fee` in the shielded pool, so the row shows that net value. Sends and receives
     * already carry the net transfer amount.
     */
    private fun getDisplayedAmount(transaction: Transaction): Zatoshi {
        val fee = transaction.fee
        return if (transaction is ShieldTransaction && fee != null && transaction.amount > fee) {
    ...
    private fun getFiatValue(
        ...
        val fiatValue = rate.toFiatString(getDisplayedAmount(transaction))
        return if (transaction is SendTransaction) {
            stringRes("- ") + fiatValue
        } else {
            fiatValue
        }
        ...
        val stringRes =
            when (data.transaction) {
                ...
                is SendTransaction.Pending -> {
                    stringRes("- ") + stringRes(getDisplayedAmount(data.transaction))
                }
                ...
                is ReceiveTransaction.Failed,
                is ReceiveTransaction.Pending -> {
                    stringRes(getDisplayedAmount(data.transaction))
                }
            }
