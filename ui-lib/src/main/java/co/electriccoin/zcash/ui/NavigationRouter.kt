...
class NavigationRouterImpl : NavigationRouter {
    ...
    // A cold-start intent can arrive before RootNavGraph starts collecting.
    // Buffer it so notification and external deep links are delivered once the
    // navigation host is ready instead of being dropped by trySend().
    private val channel = Channel<BaseNavigationCommand>(capacity = Channel.BUFFERED)
