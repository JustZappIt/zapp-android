...
class MigrationWorkerFailureRoutingTest {
    @Test
    fun `InvalidNote calls notifyMigrationPlanInvalid, cancels Lane A, returns success`() =
        runTest {
    ...
    @Test
    fun `InvalidNote cancels Lane A even when plan is null`() =
        runTest {
            val notifier = mockk<TestMigrationNotifier> { coJustRun { notifyMigrationPlanInvalid(any()) } }
            val torStore = mockk<TestTorFailureStore>()
            val laneA = mockk<TestLaneACanceller> { coJustRun { cancel(any()) } }
    ...
    @Test
    fun `TransferExpired calls notifyTransferExpired, cancels Lane A, returns success`() =
        runTest {
    ...
    @Test
    fun `TransferExpired cancels Lane A even when plan is null`() =
        runTest {
            val notifier = mockk<TestMigrationNotifier> { coJustRun { notifyTransferExpired(any()) } }
            val torStore = mockk<TestTorFailureStore>()
            val laneA = mockk<TestLaneACanceller> { coJustRun { cancel(any()) } }
